...
export async function handlerVideosRetrieve(cfg: ApiConfig, req: Request) {
  ...
  const presignedVideos = await Promise.all(videos.map(video => dbVideoToSignedVideo(cfg, video))); 
  //maps each video in the array to a presigned and then awaits the whole batch rather than individual links being await
